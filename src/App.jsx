  ...
  async function loadNews({
    ...
    try {
      ...
      */
      ...
      // If no results and query looks like Ukrainian (Cyrillic), try transliterated query
      if (validArticles.length === 0 && /[\u0400-\u04FF]/.test(cleanQuery)) {
        const translit = transliterateUAToEN(cleanQuery);
        if (translit && translit !== cleanQuery) {
          try {
            const retry = await fetchNews({
              q: translit,
              page,
              pageSize: 12,
              country: "",
            ...
            const retryArticles = Array.isArray(retry?.articles) ? retry.articles : [];
            const retryValid = retryArticles.filter(a => a && a.url && a.title);

            if (retryValid.length > 0) {
              setNews(retryValid);
              newsCacheRef.current[cacheKey] = retryValid;
              return;
            }
          } catch (e) {
            console.warn("Retry transliterated search failed", e);
          }
        }
      }
        ...
        <div className="container news-page">
          ...
          <div className="news-pagination arrows">
            ...
            <button
              className="arrow-btn"
              onClick={goNextNews}
              disabled={news.length === 0 || newsLoading}
